...
import {Users, UserPlus, UserCheck, UserRoundPen, MessagesSquare, MessageSquare} from 'lucide-react'
      ...
      <div className='max-w-6xl mx-auto p-6'>
        <div className='mb-8'>
          ...
          <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
            {
              dataArray.map((tab)=>(
                <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`cursor-pointer flex items-center px-3 py-3 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
                ...
                <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                  ...
                  <div className='flex-1'>
                    ...
                    <div className='flex max-sm:flex-col gap-2 mt-4'>
                      {
                        <button onClick={()=>navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer'>
                            View Profile
                        </button>
                      }
                      {
